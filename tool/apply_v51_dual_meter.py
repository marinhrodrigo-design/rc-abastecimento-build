...
new="""    final smId=_intOrNull(sm?['id']);\n    final smMeasurement='${smId==null?sm?['measurement_type']:(measurementOverridesV51[smId]??sm?['measurement_type'])??''}'.trim().toLowerCase();\n    final measurementNotApplicable=sm!=null&&st==null&&smMeasurement=='none';"""
assert old in s
s=s.replace(old,new,1)
p.write_text(s)
